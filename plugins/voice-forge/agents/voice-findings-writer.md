...
Read `${CLAUDE_PLUGIN_ROOT}/references/findings-template.md` for the required structure before writing anything.
Read `${CLAUDE_PLUGIN_ROOT}/references/lessons-learned.md` before writing — the caveats in rules 4 and 5 must appear in the doc.
...
- `${CLAUDE_PLUGIN_ROOT}/references/findings-template.md` — structure to follow
...
Follow the template structure from `${CLAUDE_PLUGIN_ROOT}/references/findings-template.md`. Fill each section from the loaded data:
...
**Caveats section** (mandatory — do not omit): include the caveats from `${CLAUDE_PLUGIN_ROOT}/references/lessons-learned.md` Rules 4 and 5, verbatim. Do not paraphrase.
