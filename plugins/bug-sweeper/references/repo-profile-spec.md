...
### How bug-sweeper applies this

bug-sweeper reads three keys:

| Key | Use |
|-----|-----|
| `project_roots.backend` | Directory holding the Node.js manifest. `npm run build` and `npm audit` run here, not at the repo root. |
| `project_roots.ios` / `.android` | Additional review surfaces, each reviewed by its own reviewer agent. |
| `languages` | Constrains the reviewer's file filter. Overridden by `--languages`. |

bug-sweeper never writes to a branch, so no branch keys are read.

**No manifest anywhere is the only hard failure.** A manifest in a
subdirectory is the normal monorepo case, resolved via `--project-root`, the
profile, or `git ls-files '*package.json'` discovery.
...
For bug-sweeper, a missing profile is usually harmless: manifest discovery
handles the single-manifest case on its own. It matters when several manifests
sit at the same depth, where discovery cannot choose between them.

The failure message must name **both** remedies:

```
bug-sweeper: multiple candidate project roots at the same depth:
  api/package.json
  web/package.json
  Disambiguate with --project-root <dir>, or set project_roots.backend
  in .claude/repo-profile.md.
```
