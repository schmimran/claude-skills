...
A branch name has no safe default. When `trunk` or `release_target` cannot be resolved from a
flag or the profile, the pipeline stops and names both remedies. It never falls back to the
repo's default branch, and it never guesses.
...
```
ERROR: Could not resolve the integration branch.

Remedy one — commit a repo profile at .claude/repo-profile.md containing:

    trunk: <branch>

Remedy two — pass the branch explicitly:

    /feature-creator <owner/repo> --integration-branch <branch>
```
