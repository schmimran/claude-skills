...
paths.  Read the file with the `Read` tool, then iterate over its lines:

```bash
# Alternatively, read the file list directly in Bash:
cat "${TRACKED_FILES_PATH}"
```

If for some reason `TRACKED_FILES_PATH` is unavailable, fall back to:

```bash
git -C "$REPO_DIR" ls-files --cached
```

Do **not** fall back to bare `find` — this plugin requires git and `find`
