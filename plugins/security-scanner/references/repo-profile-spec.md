...
### How security-scanner applies this

security-scanner reads one key: **`project_roots.backend`**, the directory
holding the Node.js manifest. `npm audit` resolves its lockfile relative to the
working directory, so it must run there rather than at the repo root. The same
directory is used to detect `@supabase/supabase-js`.

The static scanners (semgrep, nodejsscan) deliberately scan from the **repo
root**, so Swift, Kotlin, and every other source tree stay in scope.

security-scanner never writes to a branch, so no branch keys are read.
...
For security-scanner, a missing profile is usually harmless: manifest
discovery handles the single-manifest case on its own. It matters when several
manifests sit at the same depth.

The failure message must name **both** remedies:

```
security-scanner: multiple candidate project roots at the same depth:
  api/package.json
  web/package.json
